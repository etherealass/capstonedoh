@foreach($pat as $pats)
    @if(in_array($pats->department_id,'!=',$user_dept) || Auth::user()->user_role->first()->name == 'Superadmin' || Auth::user()->user_role->first()->name == 'Admin')
        @foreach($transfers as $trans)
          @if($trans->status == "" && $pats->status == 'For Transfer')           
            <div class="row">
          <!--for indent-->
        ...
        <!--For Transfer queue-->
        <!--Graduate-->
        @elseif(in_array($pats->department_id,$user_dept) || Auth::user()->user_role->first()->name == 'Superadmin' || Auth::user()->user_role->first()->name == 'Admin')
         @if($trans->status == "" && $pats->status == 'For Transfer')           
            <div class="row">
          <!--for indent-->
              @if(Auth::user()->user_role->name == 'Superadmin' || Auth::user()->user_role->name == 'Admin')
        ...
        <div class="row">
          <div class="col-md-4" style="margin-left: 28px;margin-right: 60px">
            <p><h3 style="color:#343a40">Admission No.</h3><h6> {{$pats->admission_no}}</h6><h4 style="color:#343a40"> Patient Status: <span class="text-success">Graduated</span></p>
          </div>
          <div class="col-md-3">
          </div>
