...
class LoginController extends Controller
{
    ...
    public function getProfile()
    {
      ...
      if(Auth::user()->user_role()->first()->name == 'Superadmin'){
        return view('superadmin.index', compact('chart'))->with('roles',$roles)->with('deps',$deps)->with('users',$users)->with('transfer',$transfer)->with('graduate',$graduate)->with('pat',$pat)->with('patx',$patx)->with('patz',$patz)->with('dept_name', $department)->with('user_dept', $depts);
      }
      else if(Auth::user()->user_role()->first()->name == 'Admin'){
        return view('superadmin.index', compact('chart'))->with('roles',$roles)->with('deps',$deps)->with('users',$users)->with('transfer',$transfer)->with('graduate',$graduate)->with('pat',$pat)->with('patx',$patx)->with('patz',$patz)->with('dept_name', $department)->with('user_dept', $depts);
      }
      else if(Auth::user()->user_role()->first()->name == 'Social Worker'){
         return view('socialworker.index', compact('chart'))->with('roles',$roles)->with('deps',$deps)->with('users',$users)->with('transfer',$transfer)->with('graduate',$graduate)->with('pat',$pat)->with('patx',$patx)->with('patz',$patz)->with('dept_name', $department)->with('user_dept', $depts);
         ...
         return view('doctor.index')->with('roles',$roles)->with('deps',$deps)->with('users',$users)->with('dept_name', $department)->with('user_dept', $depts);
